...
console.log('contracts from deployemt', Object.keys(deployment.contracts))
...
for (let contractName of Object.keys(deployment.contracts)) {
  ...
  try {
    ...
    localJson = JSON.parse(f)
    console.log('localJsonBeforeChange ', Object.keys(localJson))
  } catch (e) {
  ...
  console.log('localJsonAfterChange ', Object.keys(localJson))

  // save modified JSON to build-folder
  try {
    fs.writeFileSync(path, JSON.stringify(localJson))
  } catch (e) {
