    ...
    describe('In assessment stage', function() {
        ...
        describe('Called', function() {
            ...
            describe("When assessors confirm", async () => {
                ...
                it("they should be rejected if they have not been called", async() => {
                    var balanceBefore = await aha.balanceOf.call(outsideUser);
                    await assessmentContract.confirmAssessor({from: outsideUser});
                    var balanceAfter = await aha.balanceOf.call(outsideUser);
                ...
                it("they should be charged", async () => {
                    ...
                    var balanceAfter = await aha.balanceOf.call(confirmedAssessors[0]);
        ...
        describe("Confirmed", function() {
            ...
            it("should move to committed stage when all commited", async () => {
                ...
                let receiptFromLastCommit = txResult.receipt;
                gasCosts.push({
                    function: "commit",
                    cost: {
        ...
        describe("Committed", function() {
            it("committed assessors can reveal their scores", async () => {
                ...
                await chain.revealAssessors(
                    confirmedAssessors.slice(1, nAssessors),
            ...
            it("should move to the done stage when all assessors have revealed", async() => {
                let result = await assessmentContract.reveal(
                    scores[0],
                    salts[0],
                    {from: confirmedAssessors[0]}
                );

                let receiptFromLastReveal = result.receipt;
                gasCosts.push(
                    {function: "last Reveal + calculate + payout",
                    cost: {
