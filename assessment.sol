...
contract Assessment
{
  ...
  */
  function setAssessorPool(uint seed) onlyConceptAssessment()
  {
    ...
    {
      Concept(concept).getAssessors(size*20, seed, address(this)); //If the requested pool size is smaller that the number of users in the system call function in concept to obtain assessors
    }
  }
  ...
  { //NOTE: The front end should automatically hash the assessors submitted score with a random bytes16 and submit it to this function
  ...
  { //NOTE: This function should be automatically called from the front end
    if(block.number - startTime <= 10) //If less than 10 blocks have passed since the socre commiting stage ended
    {
      bytes32 hash = sha3(score,salt); //Hashes the provided components
      if(commits[assessor] == hash) //Checks if the newly created hash is the same ad the one that the assessor provided in the parameter previously provided
      {
        if(msg.sender == assessor) //If it was correct and submitted by the assessor in the parameter
        {
          scores[msg.sender] = score; //That score is mapped to the assessor
          assessorState[assessor] = State.Done; //The assessor's state is set to done
  ...
  function calculateResult() onlyThis()
  {
    ...
    int meanAbsoluteDeviation = Math(math).calculateMAD(score,int(finalAssessors.length)); //Caclulates the MAD of the scores
